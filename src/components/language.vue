  ...
  setup() {
    ...
    const currentFlag = computed(() => {
      return currentLocale.value === 'ru'
        ? 'public/russia_twlp5xbebtio.svg'
        : 'public/united_kingdom_nsuhf1vy7yn3.svg'
    })
...
<template>
  <div class="dropdown" style="">
    <button class="dropdown__btn" type="button" @click="toggleDropdown">
      ...
      <span class="dropdown__btn-text" style="color: var(--background-color-luna)">{{
        currentLocale.toUpperCase()
      }}</span>
      <svg
        class="dropdown__btn-icon"
        :class="{ 'dropdown__btn-icon--rotated': isDropdownOpen }"
        version="1.1"
      ...
      >
        <path
          style="fill: var(--background-color-luna)"
    ...
    <ul
      ...
      :class="{ 'dropdown__list--active': isDropdownOpen }"
      style="
        background-color: var(--background-color-luna);
